...
function sparkBurst(x, y, count, maxSpeed) {
  ...
  // Emit particle FX (particles.js hooks this)
  if (window._sparkBurst_patched && window.CubeParticles) {
    const sc = { x, y };
    ...
    const sx = (x + halfW) * vScale;
    const sy = (y + halfH) * vScale;
    const colors = [
      { r: 103, g: 215, b: 240 },
      { r: 166, g: 224, b:  44 },
      { r: 250, g:  36, b: 115 },
    ...
    if (count >= 8) {
      window.CubeParticles.emitDestroyFX(sx, sy, color);
    } else {
      window.CubeParticles.addLight(sx, sy, color, 100, 0.9, 0.5);
    }
  }
}
...
function setupCanvases() {
  ...
  function frameHandler(timestamp) {
    ...
    // If game is paused, in the main menu, or on the score/game-over screen,
...
function handleCanvasPointerMove(x, y) {
  if (pointerIsDown) {
    pointerScreen.x = x;
    pointerScreen.y = y;
    // Emit slash trail particles
    if (window.CubeParticles) {
      window.CubeParticles.emitSlashTrail(x, y);
    }
  }
}
...
function unlockReward(reward){
  if(!player.rewards.includes(reward)){
    player.rewards.push(reward);
    applyRewardInGame(reward);
    console.log("Unlocked:", reward);
  }
}

function applyRewardInGame(reward){
  if(typeof playerCube !== "undefined"){
    switch(reward){
      case "trophy_red":
        playerCube.material.color.set(0xff0000);
        break;
      case "trophy_blue":
        playerCube.material.color.set(0x0000ff);
        break;
      case "trophy_legend":
        playerCube.scale.set(2,2,2);
        break;
      case "xp_speed":
        if(player.speed) player.speed += 2;
        break;
    }
  }
}
