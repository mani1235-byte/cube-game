...
(function () {
  ...
  waitForGame(() => {
    ...
    // ── 0. SKIN COLOR — tint cubes by the equipped skin ────────────────────
    // Mirrors the same approach cube-evolution.js uses for its rainbow stage:
    // mutate each poly's color right before the real draw() runs, then put
    // it back afterward. Special PINK ("strong" cube) and wireframe BLUE
    // (slowmo trigger) cubes are left alone so their gameplay meaning stays
    // readable. Loads after cube-evolution.js, so this wraps around it —
    // the evolution stage's own effects (e.g. Rainbow) still take over on
    // top of a skin when active, the same way they would with no skin at all.
    if (typeof draw === "function") {
      const _origDrawForSkin = draw;
      window.draw = function (ctx, width, height, viewScale) {
        const colors = window.cgItems.getActiveSkinColors();
        if (colors && typeof allPolys !== "undefined") {
          allPolys.forEach(p => {
            if (p.wireframe) return;
            const c = p.color;
            const isPink = typeof PINK !== "undefined" && c.r === PINK.r && c.g === PINK.g && c.b === PINK.b;
            const isBlue = typeof BLUE !== "undefined" && c.r === BLUE.r && c.g === BLUE.g && c.b === BLUE.b;
            if (isPink || isBlue) return;
            const idx = Math.abs(c.r * 3 + c.g * 5 + c.b * 7) % colors.length;
            p._cgOrigColor = c;
            p.color = colors[idx];
          });
        }
        _origDrawForSkin(ctx, width, height, viewScale);
        if (colors && typeof allPolys !== "undefined") {
          allPolys.forEach(p => {
            if (p._cgOrigColor) { p.color = p._cgOrigColor; p._cgOrigColor = null; }
          });
        }
      };
    }

    // ── 0b. TIME WARP — actually extend the session timer when owned ──────
    // Previously this only showed a toast; getSessionLimit() governs the
    // real countdown in tick(), so patch that instead.
    if (typeof getSessionLimit === "function") {
      const _origGetSessionLimit = getSessionLimit;
      window.getSessionLimit = function () {
        const base = _origGetSessionLimit();
        if (base === Infinity) return base;
        return window.cgItems.hasItem("power_time") ? base + 30 : base;
      };
    }

    // ── 0c. EXPLOSION FX — a genuinely bigger burst when owned ─────────────
    // Previously nothing gated this; every player already got the same
    // destroy burst for free, so owning the item changed nothing.
    if (window.CubeParticles && window.CubeParticles.emitDestroyFX) {
      const _origDestroyFX = window.CubeParticles.emitDestroyFX;
      window.CubeParticles.emitDestroyFX = function (x, y, color) {
        _origDestroyFX(x, y, color);
        if (window.cgItems.hasItem("power_explode")) {
          _origDestroyFX(x + (Math.random() - 0.5) * 24, y + (Math.random() - 0.5) * 24, color);
        }
      };
    }
    ...
    window.cgPowerState = {
      ...
      hasGhost()       { return window.cgItems.hasItem("power_ghost2") && _ghostPhasesLeft > 0; },
      useGhost()       { _ghostPhasesLeft = Math.max(0, _ghostPhasesLeft - 1); },
      ghostPhasesLeft(){ return _ghostPhasesLeft; },
    };

    function applyPowerUps() {
      _shieldUsedThisGame = false;
      _ghostPhasesLeft = window.cgItems.hasItem("power_ghost2") ? 3 : 0;
      window._cgFreezeActive = false;
      ...
      // power_explode: bigger burst handled by the CubeParticles patch above
      // power_ghost2: passthrough handled below in the loseHeart hook
    }

    // ── 3. SHIELD / PHASE THROUGH — intercept loseHeart ────────────────────
    if (typeof loseHeart === "function") {
      ...
      window.loseHeart = function () {
        ...
        if (window.cgPowerState.hasGhost()) {
          window.cgPowerState.useGhost();
          showPowerUpToast(`🌀 Phase Through! (${window.cgPowerState.ghostPhasesLeft()} left)`);
          const flash = document.createElement("div");
          flash.style.cssText = "position:fixed;inset:0;background:rgba(156,39,176,0.25);z-index:9998;pointer-events:none;animation:heartFlash 0.4s ease forwards";
          document.body.appendChild(flash);
          setTimeout(() => flash.remove(), 400);
          return; // block the hit
        }
    ...
    function showFreezeButton() {
      ...
      btn.addEventListener("click", () => {
        ...
        window._cgFreezeActive = true;
        setTimeout(() => { window._cgFreezeActive = false; }, 5000);
      });
